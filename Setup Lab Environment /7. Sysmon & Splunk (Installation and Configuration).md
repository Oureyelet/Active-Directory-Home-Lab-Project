...
<b>Now we have our Sysmon installed and Splunk Universal Forwarder installed along wiht our updated inputs.conf file now we can finalize our Splunk server configuration ;et's head over to our Splunk web portal and log in using the credentials that we created during the Splunk install on our server:</b>
